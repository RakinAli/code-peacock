...
- Verify before committing (`node --check` and `bash -n` only parse their first
  ...
  `bash scripts/strut.sh && for f in scripts/*.mjs scripts/lib/*.mjs; do node --check "$f"; done && for f in scripts/*.sh; do bash -n "$f"; done && bash tests/project-checks.sh && bash tests/auth.sh && bash tests/affected-routes.sh && bash tests/diff-coverage.sh && bash tests/serve.sh && claude plugin validate .`
